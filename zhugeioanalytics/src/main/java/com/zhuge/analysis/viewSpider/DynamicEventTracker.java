...
/**
 * Handles translating events detected by ViewVisitors into events sent to Zhuge
...
/* package */ class DynamicEventTracker implements ViewVisitor.OnEventListener {
    ...
    public DynamicEventTracker(CodeLessConfig codeless, Handler homeHandler) {
        mZhuge = codeless;
    ...
    public void OnEvent(View v, String eventName, boolean debounce) {
        ...
        if (debounce) {
            ...
            mZhuge.track(eventName, properties);
        }
    }
            ...
            synchronized (mDebouncedEvents) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (now - val.timeSentMillis > DEBOUNCE_TIME_MILLIS) {
                        mZhuge.track(val.eventName, val.properties);
                        iter.remove();
                    }
                }
    ...
    private final CodeLessConfig mZhuge;
    ...
    private static String LOGTAG = "Zhuge.DEventTracker";
}

 ...
 */
class ZhugeioAnalyticsWebViewMethodVisitor extends AdviceAdapter implements Opcodes {
    ...
    ZhugeioAnalyticsWebViewMethodVisitor(MethodVisitor mv, int access, String name, String desc, ZhugeioAnalyticsTransformHelper transformHelper, String className, String superName) {
        super(ZhugeioAnalyticsUtil.ASM_VERSION, mv, access, name, desc)
            ...
            if (opcode != INVOKESTATIC && TARGET_NAME_DESC.contains(name + desc)) {
                ...
                if (isAssignableWebView(owner)) {
                    ...
                    if (isDCloud(owner, className)) {
                        ...
                        mv.visitMethodInsn(INVOKESTATIC, ZhugeioAnalyticsHookConfig.ZHUGEIO_ANALYTICS_API, name + "2", desc, false)
                    }
                    return
                }
            }
